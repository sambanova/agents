<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 overflow-y-auto">
    <div class="flex min-h-screen items-center justify-center p-4">
      ...
      <!-- Modal -->
      <div
        class="relative w-full max-w-lg bg-white rounded-xl shadow-lg p-6"
        ref="trapRef"
        role="dialog"
      >
        <div class="flex justify-between items-center mb-6">
          <h2 class="text-2xl font-semibold text-primary-brandTextPrimary">
            API Settings
          </h2>
          <button
            @click="close"
            class="text-primary-brandTextSecondary hover:text-gray-700 rounded focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-primary-brandColor"
          >
            <svg
              class="w-6 h-6"
              fill="none"
        ...
        <div class="space-y-6">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">
              ...
              <a
                ...
                target="_blank"
                class="text-primary-600 hover:text-primary-700 ml-2 text-sm rounded focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-brandColor"
              >
                Get Key →
              </a>
            </label>
            <div class="relative">
              <input
                ...
                class="block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:outline-none focus:border-primary-500 pr-10"
              />
              <button
                @click="toggleSambanovaKeyVisibility"
                class="absolute inset-y-0 right-0 px-3 flex items-center text-primary-brandTextSecondary hover:text-gray-700 rounded-md focus:outline-2 focus:outline-primary-brandColor"
              >
                <EyeIcon v-if="sambanovaKeyVisible" />
                <EyeSlashIcon v-else />
              </button>
            </div>
            ...
            <div class="flex justify-end space-x-2 mt-2">
              <button
                ...
                class="px-3 py-1 text-sm border border-primary-brandBorder text-primary-brandColor rounded focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-brandColor"
              >
                Clear Key
              </button>
              <button
                @click="saveSambanovaKey"
                class="px-3 py-1 text-sm bg-primary-brandColor text-white rounded focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-brandColor"
              >
                Save Key
              </button>
            </div>
          ...
          <div v-if="isUserKeysEnabled">
            <label class="block text-sm font-medium text-gray-700 mb-1">
              Exa API Key
              <span v-if="missingKeys.exa" class="text-red-500 text-sm mt-1"
                >(*Required Key)</span
              >
            ...
            <div class="relative">
              <input
                ...
                class="block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 pr-10"
              />
              <button
            ...
            <div class="flex justify-end space-x-2 mt-2">
              <button
                @click="clearExaKey"
                class="px-3 py-1 text-sm border border-primary-brandBorder text-primary-brandColor rounded focus:outline-none"
              >
                Clear Key
              </button>
              <button
          ...
          <div v-if="isUserKeysEnabled">
            <label class="block text-sm font-medium text-gray-700 mb-1">
              Serper API Key
              <span v-if="missingKeys.serper" class="text-red-500 text-sm mt-1"
                >(*Required Key)</span
              >
              <a
          ...
          <div v-if="isUserKeysEnabled">
            ...
            <div class="relative">
              <input
                ...
                class="block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:outline-none focus:border focus:border-primary-brandColor pr-10"
              />
              <button
                @click="toggleFireworksKeyVisibility"
                class="absolute inset-y-0 right-0 px-3 flex items-center text-primary-brandTextSecondary hover:text-gray-700"
              >
                <EyeIcon v-if="fireworksKeyVisible" />
                <EyeSlashIcon v-else />
              </button>
            </div>
            <!-- Add Save and Clear Buttons for Serper -->
          ...
          <div class="mt-6 border-t pt-4 flex flex-col">
            ...
            <select
              ...
              class="block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:outline-none focus:ring-2 focus:ring-primary-500"
            >
          ...
          <div class="mt-6 border-t flex flex-row justify-between pt-4">
            ...
            <div class="mt-6 flex flex-row items-center pt-4">
              <a
                class="text-sm underline focus:outline-primary-brandColor focus:outline-offset-2"
          ...
          <div class="mt-8 pt-6 border-t border-gray-200">
            ...
            <div class="flex justify-start space-x-2 mt-2">
              <button
                ...
                class="inline-flex items-center px-3 py-1 text-sm bg-red-500 text-white rounded focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-brandColor focus-visible:ring-offset-2"
              >
                Delete Account
              </button>
            </div>
        ...
        >
          <div class="flex min-h-screen items-center justify-center p-4">
            <div
              class="relative w-full max-w-md bg-white rounded-lg shadow-lg p-6"
            >
              <div class="text-center">
                <h3
                  class="mt-4 text-lg font-medium text-primary-brandTextPrimary"
                >
                  Confirm Data Deletion
                </h3>
                <p
                  class="mt-2 text-sm text-left text-primary-brandTextSecondary"
                >
                  Are you sure you want to delete all your data? This will
                  permanently delete all your conversations, documents, and API
                  keys. This action cannot be undone and will log you out.
                </p>
                <div class="mt-6 flex justify-center space-x-4">
                  ...
                  <button
                    ...
                    class="inline-flex items-center px-3 py-1 text-sm bg-primary-brandColor text-white rounded focus:outline-none"
                    :disabled="isDeleting"
                  >
                    <svg
                      v-if="isDeleting"
...
const loadKeys = async () => {
  ...
  try {
    ...
    const savedSambanovaKey = localStorage.getItem(
      `sambanova_key_${userId.value}`
    );
    console.log(
      'Try savedExaKey,savedSerperKey,savedFireworksKey',
      savedExaKey,
      savedSerperKey,
      savedFireworksKey
    );
...
const executeDeleteAccount = async () => {
  try {
    ...
    if (response.status === 200) {
      ...
      successMessage.value =
        'Account data deleted successfully. Logging out...';
