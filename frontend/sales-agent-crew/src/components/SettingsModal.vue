    ...
    <div class="flex min-h-screen items-center justify-center p-4">
      ...
      <div class="relative w-full max-w-lg bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
        <div class="flex justify-between items-center mb-6">
          <h2 class="text-2xl font-semibold text-primary-brandTextPrimary dark:text-white">
            API Settings
          </h2>
          <button @click="close" class="text-primary-brandTextSecondary dark:text-gray-300 hover:text-gray-700 dark:hover:text-gray-400 transition-colors">
            <svg class="w-6 h-6 fill-current text-primary-brandTextSecondary dark:text-gray-300" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div
          v-if="errorMessage"
          class="mb-4 p-3 bg-red-100 dark:bg-red-900 text-red-700 dark:text-red-300 rounded"
        >
          {{ errorMessage }}
        </div>
        ...
        <div
          v-if="successMessage"
          class="mb-4 p-3 bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 rounded"
        >
          {{ successMessage }}
        </div>
        ...
        <div class="space-y-6">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              SambaNova API Key
              <span v-if="missingKeys.sambanova" class="text-red-500 text-sm mt-1">(*Required Key)</span>
              <a
            ...
            <div class="relative">
              <input
                ...
                class="block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm p-2 focus:outline-none focus:ring-primary-500 focus:border-primary-500 pr-10 bg-white dark:bg-gray-700 dark:text-gray-100"
              />
              <button
                @click="toggleSambanovaKeyVisibility"
                class="absolute inset-y-0 right-0 px-3 flex items-center text-primary-brandTextSecondary dark:text-gray-300 hover:text-gray-700 dark:hover:text-gray-400 transition-colors"
              >
                <svg
                  v-if="sambanovaKeyVisible"
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-5 w-5 fill-none stroke-current text-primary-brandTextSecondary dark:text-gray-300"
                  viewBox="0 0 24 24"
                >
                  <!-- Eye Open Icon -->
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.519 5 12 5c4.481 0 8.268 2.943 9.542 7" />
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12c1.508 4.057 5.294 7 9.542 7s8.034-2.943 9.542-7" />
                </svg>
                <svg
                  v-else
                  xmlns="http://www.w3.org/2000/svg"
            ...
            <div class="flex justify-end space-x-2 mt-2">
              <button
                ...
                class="px-3 py-1 text-sm border border-primary-brandBorder dark:border-gray-600 text-primary-brandColor dark:text-primary-brandColor rounded focus:outline-none"
              >
                Clear Key
              </button>
              <button
                @click="saveSambanovaKey"
                class="px-3 py-1 text-sm bg-primary-brandColor dark:bg-primary-brandColor text-white rounded focus:outline-none"
              >
                Save Key
              </button>
            </div>
          ...
          <div v-if="isUserKeysEnabled">
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              Exa API Key
              <span v-if="missingKeys.exa" class="text-red-500 text-sm mt-1">(*Required Key)</span>
              <a
                href="https://exa.ai/"
                target="_blank"
                class="text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 ml-2 text-sm"
          ...
          <div v-if="isUserKeysEnabled">
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              Serper API Key
              <span v-if="missingKeys.serper" class="text-red-500 text-sm mt-1">(*Required Key)</span>
              <a
          ...
          <div v-if="isUserKeysEnabled">
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
              Fireworks API Key
              <span v-if="missingKeys.fireworks" class="text-red-500 text-sm mt-1">(*Required Key)</span>
              <a
          ...
          <!-- Model Selection -->
          <div class="mt-6 border-t border-gray-200 dark:border-gray-600 pt-4 flex flex-col">
            ...
            <select
              ...
              class="block w-full border border-gray-300 dark:border-gray-600 rounded-md shadow-sm p-2 focus:outline-none focus:ring-primary-500 focus:border-primary-500 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-100"
            >
              <option value="llama-3.3-70b">
                Meta-Llama-3.3-70B-Instruct - 128K
              </option>
              <option value="deepseek-r1">DeepSeek-R1 - 8K</option>
              <option value="llama-3.1-405b">
                Meta-Llama-3.1-405B-Instruct - 16K
              </option>
            </select>
          </div>

          <div class="mt-6 border-t border-gray-200 dark:border-gray-600 flex flex-row justify-between pt-4">
            ...
            <div class="mt-6 flex flex-row items-center pt-4">
              <a
                class="text-sm underline text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300"
                href="https://community.sambanova.ai/c/agents/87"
                target="_blank"
              >
                FAQ (SN Community)
              </a>
          ...
          <div class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-600">
            ...
            <div class="flex justify-end space-x-2 mt-2">
              <button
                ...
                class="inline-flex items-center px-3 py-1 text-sm bg-primary-brandColor dark:bg-primary-brandColor text-white rounded focus:outline-none"
              >
                Delete Account
              </button>
            </div>
          ...
          <div class="flex min-h-screen items-center justify-center p-4">
            <div class="relative w-full max-w-md bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
              <div class="text-center">
                <h3 class="mt-4 text-lg font-medium text-primary-brandTextPrimary dark:text-white">
                  Confirm Data Deletion
                </h3>
                <p class="mt-2 text-sm text-gray-500 dark:text-gray-400 text-primary-brandTextSecondary dark:text-gray-400 text-left">
...
const clearSambanovaKey = async () => {
  try {
    ...
    console.log("sambanova clear error", e)
  }
  sambanovaKey.value = ''
