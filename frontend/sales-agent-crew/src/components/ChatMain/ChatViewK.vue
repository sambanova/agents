...
async function filterChat(msgData) {
  messagesData.value = msgData.messages
    .map(message => {
      ...
      if ((message.event === 'agent_completion')&&(message.type==="HumanMessage"||receivedData.additional_kwargs?.agent_type.include("_end"	)||receivedData.additional_kwargs?.agent_type.include("_interrupt")) {
        return {
          event: 'agent_completion',
