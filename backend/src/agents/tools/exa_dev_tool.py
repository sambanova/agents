...
class ExaDevToolSchema(BaseModel):
    ...
    search_type: str = Field(
        default="auto", description="Search type: 'auto', 'neural', etc."
    )
    ...
    summary: bool = Field(
        default=True, description="Whether to retrieve the 'summary' field"
    )
    livecrawl: str = Field(
        default="always", description="Use 'always' for fresh results"
    )
...
class ExaDevTool(BaseTool):
    ...
    description: str = (
        "Use Exa to perform semantic search with raw HTTP POST. Returns JSON with title, url, text, summary, etc."
    )
    ...
    def _run(self, **kwargs: Any) -> Any:
        ...
        try:
            ...
            if elapsed_time > 10:
                logger.warning(
                    f"Exa Dev Tool took {elapsed_time:.2f} seconds to complete search for query: {search_query}"
                )
            else:
                logger.info(
                    f"Exa Dev Tool took {elapsed_time:.2f} seconds to complete search for query: {search_query}"
