...
from agents.tools.financial_data import (
    ...
    get_fundamental_data_insightsentry,
    get_ticker_balance_sheet_yfinance,
    get_ticker_cashflow_yfinance,
    get_ticker_dividends_yfinance,
    get_ticker_financials_yfinance,
...
def _generate_quarterly_data_with_labels(
    total_revenue_values: List[Any], net_income_values: List[Any]
) -> List[Dict[str, Any]]:
    """
    ...
    if num_quarters_to_generate > 0:
        ...
        for i in range(num_quarters_to_generate):
            ...
            current_total_rev = (
                total_revenue_values[i] if i < len(total_revenue_values) else None
            )
            current_net_inc = (
                net_income_values[i] if i < len(net_income_values) else None
            )
            ...
            quarterly_csv.append(
                {
                    "date": quarter_label,
                    "total_revenue": (
                        str(current_total_rev)
                        if current_total_rev is not None
                        else None
                    ),
                    "net_income": (
                        str(current_net_inc) if current_net_inc is not None else None
                    ),
                }
            )
...
def fundamental_analysis_tool(ticker: str) -> Dict[str, Any]:
    ...
    try:
        if bs is not None and not bs.empty:
            if (
                "Total Current Assets" in bs.index
                and "Total Current Liabilities" in bs.index
            ):
            ...
            if (
                "Total Liabilities" in bs.index
                and "Total Stockholder Equity" in bs.index
            ):
        ...
        logger.error(
            f"Error fetching advanced statement analysis data for {ticker}: {e}"
        )
    ...
    try:
        ...
        if qfin is not None and not qfin.empty:
            for date_col in qfin.columns:
                col_str = (
                    str(date_col.date()) if hasattr(date_col, "date") else str(date_col)
                )
                total_rev = None
                net_inc = None
...
def fundamental_analysis_tool_rapidapi(ticker: str) -> Dict[str, Any]:
    ...
    result = {
        ...
        "industry": info.get("industry", ""),
        "market_cap": str(
            info.get("priceToBook", 0.0)
            * info.get("bookValue", 0.0)
            * info.get("sharesOutstanding", 0.0)
        ),
    ...
    quarterly_csv = []
    if (
        "incomeStatementHistoryQuarterly" in info
        and "incomeStatementHistory" in info["incomeStatementHistoryQuarterly"]
    ):
        for quarter in info["incomeStatementHistoryQuarterly"][
...
def fundamental_analysis_tool_insightsentry(ticker: str) -> Dict[str, Any]:
    ...
    ebitda_margin = (
        info.get("data", {}).get("profitability", {}).get("ebitda_margin_current", "")
    )
    if ebitda_margin != "":
    ...
    result = {
        ...
        "pe_ratio": str(
            info.get("data", {}).get("valuation_ratios", {}).get("price_earnings", "")
        ),
        "forward_pe": str(
            info.get("data", {})
            .get("valuation_ratios", {})
            .get("price_earnings_fy", "")
        ),
        "peg_ratio": "",
        "ps_ratio": str(
            info.get("data", {})
            .get("valuation_ratios", {})
            .get("price_sales_current", "")
        ),
        "price_to_book": str(
            info.get("data", {})
            .get("valuation_ratios", {})
            .get("price_book_current", "")
        ),
        "dividend_yield": str(
            info.get("data", {}).get("dividends", {}).get("dividends_yield_current", "")
        ),
        "beta": "",
        "year_high": "",
        "year_low": "",
        ...
        "target_price": str(
            info.get("data", {})
            .get("price_targets", {})
            .get("price_target_average", "")
        ),
        "earnings_per_share": str(info.get("earnings_per_share_basic_ttm", "")),
        "profit_margins": str(
            info.get("data", {}).get("profitability", {}).get("net_margin", "")
        ),
        "operating_margins": str(
            info.get("data", {}).get("profitability", {}).get("operating_margin", "")
        ),
        "ebitda_margins": ebitda_margin,
        "short_ratio": "",
        "current_ratio": str(
            info.get("data", {}).get("balance_sheet", {}).get("current_ratio", "")
        ),
        "debt_to_equity": str(
            info.get("data", {}).get("balance_sheet", {}).get("debt_to_equity_fy", "")
        ),
        "return_on_equity": str(
            info.get("data", {}).get("profitability", {}).get("return_on_equity", "")
        ),
        "return_on_assets": str(
            info.get("data", {}).get("profitability", {}).get("return_on_assets", "")
        ),
        "revenue_growth": "",
        "free_cash_flow": str(
            info.get("data", {}).get("cash_flow", {}).get("free_cash_flow_ttm", "")
        ),
    }

    total_revenue_values = (
        info.get("data", {}).get("income_statement", {}).get("total_revenue_fq_h", [])
    )
    net_income_values = (
        info.get("data", {}).get("income_statement", {}).get("net_income_fq_h", [])
    )

    quarterly_csv = _generate_quarterly_data_with_labels(
        total_revenue_values, net_income_values
    )

    result["quarterly_fundamentals"] = quarterly_csv

    adv_data = {}
    adv_data["shares_outstanding"] = str(
        info.get("data", {})
        .get("income_statement", {})
        .get("basic_shares_outstanding_fy", "")
    )
    adv_data["float_shares"] = ""
    adv_data["enterprise_value"] = ""
    adv_data["book_value"] = str(
        info.get("data", {}).get("valuation_ratios", {}).get("book_per_share_fy", "")
    )

    div_hist = []
    dividend_ex_date_h = (
        info.get("data", {}).get("dividends", {}).get("dividend_ex_date_h", [])
    )
    dividend_amount_h = (
        info.get("data", {}).get("dividends", {}).get("dividend_amount_h", [])
    )
