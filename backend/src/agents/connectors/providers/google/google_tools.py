...
def get_file_from_redis_sync(user_id: str, file_id: str) -> Tuple[Optional[bytes], Optional[dict]]:
    """
    Synchronous wrapper to get file from Redis storage.
    Uses the sync Redis client with decryption.
    """
    try:
        ...
        # Build Redis keys (same pattern as RedisStorage class)
        ...
        # Verify file belongs to user (this is not encrypted)
        if not redis_client.sismember(user_files_key, file_id):
            logger.warning(f"File {file_id} does not belong to user {user_id}")
            return None, None

        # Get encrypted file metadata
        encrypted_metadata = redis_client.get(file_metadata_key)
        if not encrypted_metadata:
            logger.warning(f"No metadata found for file {file_id}")
            return None, None

        # Decrypt metadata
        decrypted_metadata = encryption.decrypt(encrypted_metadata, user_id)
        file_metadata = json.loads(decrypted_metadata)

        # Get encrypted file data
        encrypted_data = redis_client.get(file_data_key)
        if not encrypted_data:
            logger.warning(f"No data found for file {file_id}")
            return None, None

        # Decrypt file data
        file_bytes = encryption.decrypt(encrypted_data, user_id)

        # Convert to bytes if needed
        if isinstance(file_bytes, str):
            file_bytes = file_bytes.encode('utf-8')

        logger.info(f"Retrieved file from Redis (sync), size: {len(file_bytes)} bytes")
        return file_bytes, file_metadata

    except Exception as e:
        logger.error(f"Error fetching file from Redis: {e}", exc_info=True)
        return None, None
...
class GoogleDriveUploadInput(BaseModel):
    ...
    content: Optional[str] = Field(None, description="File content to upload (text for text files, base64-encoded for binary files, or file_id reference)")
    ...
    is_base64: bool = Field(default=False, description="Whether content is base64-encoded (for binary files)")
    file_id: Optional[str] = Field(None, description="Redis file ID to upload (for files generated by code execution)")
...
class GoogleDriveUploadTool(BaseTool):
    ...
    description: str = "Upload a file to Google Drive. For files generated by code execution (PDFs, images, etc.), use the file_id returned by the code sandbox (UUID format like '8b5cc6e5-2fd5-4021-b227-3f6cf41e8944'). For text content, provide the content directly."
    ...
    def _run(
        ...
        try:
            ...
            # Handle file_id reference (for files stored in Redis)
            if file_id and not content:
                # Check if file_id looks like a file path instead of a Redis ID
                if file_id.startswith('./') or file_id.startswith('/') or '/' in file_id:
                    logger.warning(f"file_id appears to be a file path ({file_id}), not a Redis ID. This tool requires a Redis file ID.")
                    return f"Error: file_id should be a Redis file ID (UUID format), not a file path. Received: {file_id}"

                logger.info(f"Fetching file from storage with ID: {file_id}")
                try:
                    # Use our sync wrapper to get the file
                    file_bytes, file_metadata = get_file_from_redis_sync(self.user_id, file_id)

                    if not file_bytes:
                        logger.error(f"File not found or empty for ID: {file_id}")
                        return f"Error: File with ID {file_id} not found in storage or has no content."

                    logger.info(f"Retrieved file from storage, size: {len(file_bytes)} bytes, metadata: {file_metadata}")

                except Exception as e:
                    logger.error(f"Error fetching file from storage: {e}", exc_info=True)
                    return f"Error fetching file from storage: {str(e)}"

            # Handle direct content upload
            elif content:
                # Prepare file content based on type
                if is_base64 or mime_type in ['application/pdf', 'image/png', 'image/jpeg', 'image/gif',
                                              'application/zip', 'application/octet-stream',
                                              'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
                                              'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                                              'application/vnd.openxmlformats-officedocument.presentationml.presentation']:
                    # For binary files, expect base64-encoded content
                    logger.info(f"Uploading binary file {file_name} with MIME type {mime_type}")
                    try:
                        # If content is base64-encoded, decode it
                        file_bytes = base64.b64decode(content)
                    except:
                        # If decoding fails, assume it's already raw binary or text that needs encoding
                        logger.warning(f"Failed to decode base64 for {file_name}, treating as text")
                        file_bytes = content.encode('utf-8')
                else:
                    # For text files, encode as UTF-8
                    logger.info(f"Uploading text file {file_name} with MIME type {mime_type}")
                    file_bytes = content.encode('utf-8')
            else:
                return "Error: Either 'content' or 'file_id' must be provided"
