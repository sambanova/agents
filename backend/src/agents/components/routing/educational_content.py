    ...
    def __init__(self, api_keys: APIKeys):
        ...
        logger.info(
            logger.format_message(
                None, f"Initializing EducationalContentAgent with ID: {self.id}"
            )
        )
        self.api_keys = api_keys

    @message_handler
    async def handle_educational_content_request(
        ...
        try:
            ...
            edu_inputs = {
                ...
                "audience_level": (
                    message.parameters.audience_level
                    if message.parameters.audience_level
                    else ""
                ),
                "additional_context": (
                    message.parameters.focus_areas
                    if message.parameters.focus_areas
                    else ""
                ),
            }
            if message.docs:
            ...
            usage_stats = (
                [edu_flow.research_usage]
                + edu_flow.content_usage
                + ([edu_flow.summariser_usage] if edu_flow.summariser_usage else [])
            )
            ...
            logger.info(
                logger.format_message(
                    ctx.topic_id.source, "Successfully generated educational content"
                )
            )
            sections_with_content = EducationalPlanResult.model_validate(
                {"sections": result}
            )
            ...
            logger.info(
                logger.format_message(
                    ctx.topic_id.source, "Publishing educational content to user_proxy"
                )
            )
