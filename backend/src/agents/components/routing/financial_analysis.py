...
class FinancialAnalysisAgent(RoutedAgent):
    def __init__(
        ...
        logger.info(
            logger.format_message(
                None, f"Initializing FinancialAnalysisAgent with ID: {self.id}"
            )
        )
    ...
    async def execute_financial(
        ...
        logger.info(
            logger.format_message(
                None,
                f"Extracting financial information from query: '{parameters.get('query_text', '')[:100]}...'",
            )
        )
        ...
        logger.info(
            logger.format_message(
                None,
                f"Analyzing company: {parameters.get('company_name', '')} (ticker: {parameters.get('ticker', '')})",
            )
        )
        inputs = {
        ...
        if "docs" in parameters:
            ...
            logger.info(
                logger.format_message(
                    None, "Including additional document analysis in financial analysis"
                )
            )
    ...
    @message_handler
    async def handle_financial_analysis_request(
        ...
        try:
            ...
            logger.info(
                logger.format_message(
                    ctx.topic_id.source, "Successfully parsed financial analysis result"
                )
            )
            ...
            logger.info(
                logger.format_message(
                    ctx.topic_id.source, "Publishing financial analysis to user_proxy"
                )
            )
            await self.publish_message(
                response,
