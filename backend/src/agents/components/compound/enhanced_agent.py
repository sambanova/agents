...
def get_llm(llm_type: LLMType, api_key: str = None, user_id: Optional[str] = None, api_keys: dict = None):
    ...
    if CONFIG_SYSTEM_AVAILABLE and admin_enabled and user_id and (api_keys or api_key):
        ...
        task_map = {
            LLMType.SN_MINIMAX_M2_7: "main_agent",
        ...
        if llm_type == LLMType.SN_LLAMA_3_3_70B:
        ...
        elif llm_type == LLMType.SN_MINIMAX_M2_7:
            llm = get_sambanova_llm(model="MiniMax-M2.7", api_key=api_key)
            ...
            # default to MiniMax M2.7 for backward compatibility
            logger.warning(f"Unexpected agent type: {llm_type}, defaulting to MiniMax M2.7")
            llm = get_sambanova_llm(model="MiniMax-M2.7", api_key=api_key)
...
class EnhancedConfigurableAgent(RunnableBinding):
    ...
    llm_type: LLMType = LLMType.SN_MINIMAX_M2_7
