...
async def start_mcp_server(
    ...
    Returns:
        Success message and server status
    """
    try:
        user_id = get_user_id_from_token(token_data)
        if not user_id:
        ...
        if not server_config.enabled:
            return JSONResponse(
                status_code=400,
                content={"error": "Cannot start disabled server. Enable it first."},
            )
        ...
        success = await mcp_manager.start_server(user_id, server_id)
        
        if success:
            return JSONResponse(
                status_code=200,
                content={
                    "message": f"Server '{server_config.name}' start command sent successfully",
                    "server_id": server_id,
                    "status": "starting"
                },
            )
            ...
            return JSONResponse(
                status_code=500,
                content={"error": "Failed to start server"},
            )
        ...
        return JSONResponse(
            status_code=500,
            content={"error": f"Failed to start MCP server: {str(e)}"},
        )
...
async def stop_mcp_server(
    ...
    try:
        ...
        success = await mcp_manager.stop_server(user_id, server_id)
        
        if success:
            return JSONResponse(
                status_code=200,
                content={
                    "message": f"Server '{server_config.name}' stop command sent successfully",
                    "server_id": server_id,
                    "status": "stopped"
                },
            )
            ...
            return JSONResponse(
                status_code=500,
                content={"error": "Failed to stop server"},
            )
        ...
        return JSONResponse(
            status_code=500,
            content={"error": f"Failed to stop MCP server: {str(e)}"},
        )
