...
@app.get("/files/{file_id}/public")
async def get_file_public(
    file_id: str,
    user_id: str = Query(..., description="User ID for file access"),
):
    """
    Serve a file by its ID for public access (used for Daytona-generated files).
    This endpoint is specifically for files generated by Daytona that need to be 
    displayed in the frontend without requiring authentication headers.
    """
    try:
        # First try with the provided user_id
        result = await app.state.redis_storage_service.get_file(
            user_id, file_id
        )

        # If not found and user_id is 'current_user', try to find the file across all users
        # This handles the case where attachment files don't include user_id in their reference
        if not result and user_id == "current_user":
            # Try to find the file by searching for its metadata without user restriction
            # This is a fallback for attachment files which don't include user_id
            try:
                # Get all possible metadata keys and try to find the file
                all_keys = await app.state.redis_client.keys(f"file_metadata:*:{file_id}")
                for key in all_keys:
                    if isinstance(key, bytes):
                        key = key.decode('utf-8')
                    # Extract user_id from the key format: file_metadata:{user_id}:{file_id}
                    parts = key.split(':')
                    if len(parts) == 3 and parts[2] == file_id:
                        found_user_id = parts[1]
                        result = await app.state.redis_storage_service.get_file(
                            found_user_id, file_id
                        )
                        if result:
                            logger.info(f"Found file for user_id: {found_user_id}", file_id=file_id)
                            break
            except Exception as search_error:
                logger.warning(f"Error searching for file across users: {str(search_error)}", file_id=file_id)
        ...
        file_data, file_metadata = result
        ...
        # Additional security: Only allow public access to Daytona-generated files
        if file_metadata.get("source") != "daytona":
            return JSONResponse(
                status_code=403,
                content={"error": "Public access not allowed for this file type"},
            )

        # Set appropriate MIME type based on file format
        format_lower = file_metadata.get("format", "").lower()
        mime_types = {
            "pdf": "application/pdf",
            "png": "image/png", 
            "jpg": "image/jpeg",
        ...
        media_type = mime_types.get(format_lower, file_metadata.get("format", "application/octet-stream"))
        ...
        logger.error(f"Error serving public file: {str(e)}", file_id=file_id, user_id=user_id)
        return JSONResponse(status_code=500, content={"error": str(e)})
