...
from src.agents.utils.file_storage import get_file, get_file_metadata
import jwt
import os
...
async def get_file_public_endpoint(
    ...
    try:
        ...
        metadata = await get_file_metadata(user_id, file_id)
        if not metadata:
            print(f"Public File API: File metadata not found for {file_id} and user {user_id}")
            raise HTTPException(status_code=404, detail="File not found")
        ...
        if metadata['format'].lower() not in allowed_formats:
            print(f"Public File API: File format {metadata['format']} not allowed for public access")
            raise HTTPException(status_code=403, detail="File type not allowed for public access")
