...
The system now includes a dedicated MCP Orchestrator microservice for managing Model Context Protocol (MCP) servers:

### Components

1. **Main Backend** (`backend/src/agents/api/main.py`)
   - Handles chat, authentication, and business logic
   - Delegates MCP operations to orchestrator via HTTP

2. **MCP Orchestrator** (`backend/src/orchestrator/main.py`)
   - Dedicated service for spawning and managing MCP Studio servers
   - Isolated from main API for stability and performance
   - Endpoints: `/servers/{id}/start`, `/servers/{id}/stop`, `/servers/{id}/status`
...
# Orchestrator available at http://localhost:8090
...
cd helm
make build push deploy ENV=prod

# Or deploy individually
make build-orchestrator push-orchestrator
helm upgrade aiskagents-prod ./aiskagents -f ./aiskagents/values.prod.yaml
```

### Environment Variables

- `MCP_ORCHESTRATOR_URL`: URL of orchestrator service (e.g., `http://orchestrator:8090`)
- If not set, MCP servers run locally within the main backend (fallback mode)

### Benefits

- **Isolation**: MCP server crashes don't affect the main API
- **Performance**: Persistent connections and Redis caching for sub-10ms tool discovery
- **Scalability**: Orchestrator can be scaled independently
- **Reliability**: Exponential backoff prevents restart storms
